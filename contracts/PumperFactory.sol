...
contract PumperFactory {
    ...
    function emitBuy(address buyer, uint256 amount, uint256 price) public {
        emit Buy(buyer, amount, price);
    }

    function emitSell(address seller, uint256 amount, uint256 price) public {
        emit Sell(seller, amount, price);
    }

    function emitLiquidtyDeployed(address pool, uint256 x, uint256 y) public {
